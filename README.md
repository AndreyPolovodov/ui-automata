...
- *Browser*: `TabWithAttribute` (title match on a CDP tab anchor)
- *System*: `ProcessRunning`, `FileExists`, `ExecSucceeded` (exit code 0), `EvalCondition` (boolean expression), `Always`
- *Logic*: `AllOf`, `AnyOf`, `Not`

**Actions**:

- *Interaction*: `Click`, `Invoke` (IInvokePattern — works on off-screen elements), `TypeText`, `SetValue` (ValuePattern, no keystroke simulation), `PressKey`, `ActivateWindow`
- *Data*: `Extract` (UIA attribute → output variable), `Eval` (expression → output variable), `WriteOutput`
- *System*: `Exec` (external process, capture stdout), `MoveFile`, `WriteFile`
...
Windows UI Automation exposes the full semantic structure of every application: roles, names, automation IDs, states, values, and tree relationships. An agent working through UIA knows a button is disabled, a dialog is modal, a combo box has these options — without any inference step. Where visual grounding is needed (rendered text, custom-drawn controls, apps with incomplete UIA), the agent falls back to `vision`. The two complement each other: UIA for precision and speed, vision for ground truth when required.
